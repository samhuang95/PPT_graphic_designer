...
5. **版面選擇邏輯**:
   - [V] 實作規則：若只有標題 -> 使用「標題投影片 (Title Slide)」或「章節頁 (Section Header)」。
   - [V] 實作規則：若有標題 + 少量文字 -> 使用「標題及物件 (Title and Content)」。
   - [V] 實作規則：若有標題 + 兩欄文字 -> 使用「兩欄內容 (Two Content)」。
6. **溢出處理**:
   - [V] 偵測文字是否過多，若過多則自動縮小字級或拆分為兩頁 (選用)。
